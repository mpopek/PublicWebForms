...
{
    public partial class PartnerstviOsaPomaha : System.Web.UI.Page
    {
        ...
        private string smlouvaTyp = "PARTNERSTVÍ OSA – ON-LINE ŽÁDOST (OSA POMÁHÁ)";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Request["PDF"] == "regenerate" && ConfigurationManager.AppSettings["ActivateRegeneratePDF"] == "true")
            {
                ...
                {
                    List<OSATBL_PWF_Zadost> list = (from a in db.OSATBL_PWF_PartnerstviOsaPomahas
                    ...
                    if (list != null && list.Count() > 0)
                    {
                        ...
                        Response.Redirect(Page.ResolveClientUrl("~/forms/PartnerstviOsaPomaha.aspx?PDF=regenerate"));
                    }
                }
            }
            if (Request["pdfid"] != null && Request["pdfid"].Length > 0 && ConfigurationManager.AppSettings["ActivateRegeneratePDF"] == "true")
            {
                using (dbDataContext db = new dbDataContext())
                {
                    OSATBL_PWF_PartnerstviOsaPomaha smlouva = (from a in db.OSATBL_PWF_Zadosts
                                                               join b in db.OSATBL_PWF_PartnerstviOsaPomahas on a.formGuid equals b.guid
        ...
        {
            OSATBL_PWF_PartnerstviOsaPomaha smlouva = new OSATBL_PWF_PartnerstviOsaPomaha();
            ...
            using (dbDataContext db = new dbDataContext())
            {
                try
                {
                    ...
                    db.OSATBL_PWF_PartnerstviOsaPomahas.InsertOnSubmit(smlouva);
                    db.SubmitChanges();
