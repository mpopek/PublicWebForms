...
{
    public partial class PartnerstviPremieryOsa : System.Web.UI.Page
    {
        ...
        private string smlouvaTyp = "PARTNERSTVÍ OSA – ON-LINE ŽÁDOST (PREMIÉRY OSA)";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Request["PDF"] == "regenerate" && ConfigurationManager.AppSettings["ActivateRegeneratePDF"] == "true")
            {
                ...
                {
                    List<OSATBL_PWF_Zadost> list = (from a in db.OSATBL_PWF_PartnerstviPremieryOsas
                    ...
                    if (list != null && list.Count() > 0)
                    {
                        ...
                        Response.Redirect(Page.ResolveClientUrl("~/forms/PartnerstviPremieryOsa.aspx?PDF=regenerate"));
                    }
                }
            }
            if (Request["pdfid"] != null && Request["pdfid"].Length > 0 && ConfigurationManager.AppSettings["ActivateRegeneratePDF"] == "true")
            {
                using (dbDataContext db = new dbDataContext())
                {
                    OSATBL_PWF_PartnerstviPremieryOsa smlouva = (from a in db.OSATBL_PWF_Zadosts
                                                                 join b in db.OSATBL_PWF_PartnerstviPremieryOsas on a.formGuid equals b.guid
        ...
        {
            OSATBL_PWF_PartnerstviPremieryOsa smlouva = new OSATBL_PWF_PartnerstviPremieryOsa();
            ...
            using (dbDataContext db = new dbDataContext())
            {
                try
                {
                    ...
                    db.OSATBL_PWF_PartnerstviPremieryOsas.InsertOnSubmit(smlouva);
                    db.SubmitChanges();
                    this.smlouvaID = zadost.id;
                    body.Attributes.Add("onload", "javascript:GeneratePDF('" + zadost.id.ToString() + "');");
                }
